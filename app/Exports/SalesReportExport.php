...
class SalesReportExport implements FromCollection, WithHeadings, WithColumnFormatting, WithStyles, WithEvents
{
    ...
    public function collection()
    {
        return collect($this->transactions)->map(function($transaction) {
            $transaction->total_sales_after_discount_tax = floatval(str_replace(',', '', $transaction->total_sales_after_discount_tax));
            $transaction->total_sales_return = floatval(str_replace(',', '', $transaction->total_sales_return));
    ...
    {
        return [
            ['Laporan Penjualan', '', '', '', 'Toko: ' . $this->storeName],
            ['Periode: ' . $this->startDate . ' - ' . $this->endDate],
            [],
            ['No. Invoice', 'Tanggal Transaksi', 'Total Penjualan', 'Total Retur', 'Total Bersih']
        ];
    }
