...
<div class="flex-grow-1">
    <div class="row g-6">
        <div class="col-md-6 mb-3">
            <div class="card">
                <h5 class="card-header">Tambah Bank</h5>
                <div class="card-body">
                    <form id="bankForm" class="row" action="{{ route('bank.store') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="col-12 mb-3">
                            <x-input-label for="name" :value="__('Nama Bank')" />
                            <x-text-input id="name" class="form-control" type="text" name="name" :value="old('name')" required autofocus autocomplete="name" placeholder="Masukkan nama bank" />
                            <x-input-error :messages="$errors->get('name')" class="mt-2 text-danger" />
                        </div>

                        <div class="col-12 mb-3">
                            <x-input-label for="branch" :value="__('Cabang')" />
                            <x-text-input id="branch" class="form-control" type="text" name="branch" :value="old('branch')" required placeholder="Masukkan cabang bank" />
                            <x-input-error :messages="$errors->get('branch')" class="mt-2 text-danger" />
                        </div>

                        <div class="col-12 mb-3">
                            <x-input-label for="account_number" :value="__('Nomor Rekening')" />
                            <x-text-input id="account_number" class="form-control" type="text" name="account_number" :value="old('account_number')" required placeholder="Masukkan nomor rekening" />
                        ...
                        <div class="col-12 mb-3">
                            ...
                            <x-text-input id="account_name" class="form-control" type="text" name="account_name" :value="old('account_name')" required placeholder="Masukkan nomor rekening" />
                            <x-input-error :messages="$errors->get('account_name')" class="mt-2 text-danger" />
                        </div>

                        <div class="col-12 mb-3">
                            <x-input-label for="swift_code" :value="__('Kode SWIFT')" />
                            <x-text-input id="swift_code" class="form-control" type="text" name="swift_code" :value="old('swift_code')" placeholder="Masukkan kode SWIFT" />
                            <x-input-error :messages="$errors->get('swift_code')" class="mt-2 text-danger" />
                        </div>

                        <div class="col-12 mb-3">
                            <x-input-label for="credit" :value="__('Kredit')" />
                            <x-text-input id="credit" class="form-control" type="number" name="credit" :value="old('credit')" required placeholder="Masukkan jumlah kredit" step="0.01" />
                            <x-input-error :messages="$errors->get('credit')" class="mt-2 text-danger" />
                        </div>

                        <div class="col-12 mb-3">
                            <x-input-label for="debit" :value="__('Debit')" />
                            <x-text-input id="debit" class="form-control" type="number" name="debit" :value="old('debit')" required placeholder="Masukkan jumlah debit" step="0.01" />
                            <x-input-error :messages="$errors->get('debit')" class="mt-2 text-danger" />
                        </div>

                        <div class="col-12 mb-3">
                            <x-input-label for="saldo" :value="__('Saldo')" />
                            <x-text-input id="saldo" class="form-control" type="number" name="saldo" :value="old('saldo')" required placeholder="Masukkan saldo" step="0.01" />
